...
\subtitle{[Exercise Session 1] Bisection Algorithm}
\fi
\date{}
...
\begin{enumerate}
\item Estimate a zero of $f(x)=x^2-3$ in the interval $[0,3]$\\by four iterations of the bisection algorithm
\item Evaluate the error for each iteration
\item Do the same calculations in Excel
\item How many iterations are need for an error $<0.001$?
\item How many iterations would be needed according to a pessimistic estimate? Do I actually need to execute the algorithm to answer this question?
\end{enumerate}}
\end{document}
