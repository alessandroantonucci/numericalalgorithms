...
\frame{\frametitle{\ifanswers Esercizi sul formato float \else Exercises on Float Format\fi}
\setstretch{1.4}
\begin{enumerate}
\ifanswers
...
\else
\item Read the sequences FA800000 e 42F91FE6 (32 bits zipped in hexa) according to the float format rules
\item Identify the 32-bits sequence (zipped in hexa) associated with $x=-137.12$ according to the float format rules
\item The format {\tt double} is the 64-bits analogous of float. The format uses 11 bits for the exponent. Retrace the format rules.
\fi
\end{enumerate}
}
...
\vskip 2mm
$b_1=0 \Rightarrow$ $x<0$ 
\vskip 2mm
exp = horner($1111\,0101$)-127=118
\vskip 2mm
mantissa = 1.$0$
\vskip 2mm
...
\vskip 2mm
\ifanswers Notazione scientifica \else Scientific notation \fi
$-10001001.\overline{00011110101110000100}_2 \cdot 2^0$ 
\vskip 2mm
\ifanswers Mantissa normalizzata \else Mantissa normalised \fi
...
\end{center}
\vskip 2mm
$1|10000110|00010010001111010111000$ 
\vskip 2mm
Hexa zip
\vskip 2mm
\small
