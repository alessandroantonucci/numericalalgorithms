...
\subtitle{[Lezione 1 \& 2] Algoritmi di Bracketing\\Bisezione e Regola Falsi}
\else
\title{Numerics (Part II)}
\subtitle{[Lectures 1 \& 2] Bracketing Algorithms \\Bisection and Regula Falsi}
\fi
\date{}
...
\begin{itemize}
\item \ifita Data una funzione $f$ (continua)\else Given a (continuous) function $f$ \fi
\ifita
...
\end{itemize}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frame{\frametitle{\ifita La bisezione\else Bisection Algorithm\fi}
\begin{itemize}
\item \ifita Dopo aver localizzato $x^*$ sull'intervallo $[a,b]$ \else
After having localized $x^*$ in the interval $[a,b]$ \fi
\item \ifita Prendo il punto medio \else Take the midpoint \fi $c:=\frac{a+b}{2}$
\ifita
...
\end{center}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frame{\frametitle{\ifita La bisezione (pseudo codice)\else Bisection (pseudocode) \fi}
\begin{columns}
...
\end{columns}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frame{\frametitle{\ifita Promemoria \else Don't forget\fi}
\begin{itemize} 
\item \ifita Dato l'intervallo \else Given the interval \fi $[a,b]$:
	\begin{itemize}
	\item \ifita la sua ampiezza \`e \else its size is \fi$b-a$
	\item \ifita il suo punto medio \else its midpoint is \fi $\frac{a+b}{2}$
	\end{itemize}
\ifita
...
\frame{\frametitle{\ifita Analisi precisione \else Precision\fi}
\begin{itemize}
\item \ifita Intervallo iniziale \else Initial interval \fi $[a^0,b^0]$, 
\ifita stima puntuale \else point estimate \fi $c^0:=\frac{a^0+b^0}{2}$
\item $x^* \in [a^0,b^0]$ e $\epsilon^0:=|x^*-c^0|< \frac{b^0-a^0}{2}$
\item \ifita Analogamente \else Similarly \fi, $\epsilon^k < \frac{b^0-a^0}{2^{k+1}}$
\end{itemize}
\begin{center}
\ifita
...
\frame{\frametitle{\ifita Osservazione \else Remark \fi}
\begin{itemize}
\ifita
...
\begin{itemize}
\item \ifita Variante dell'algoritmo di bisezione \else A variant of bisection \fi 
\ifita
...
\frame{\frametitle{\ifita Ibrido Regula Falsi + Bisezione \else Hybrid Regula Falsi + Bisection\fi}
	\begin{itemize}
\ifita
